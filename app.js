...
(function(){
  ...
  var sl=document.getElementById('solList');
  if(sl&&cms.solutions){var is=pub(cms.solutions); if(is.length) sl.innerHTML=is.map(function(x){var bc=/метод/i.test(x.badge||'')?'flag':'green'; return '<div class="scn"'+(x.anchor?' id="'+esc(x.anchor)+'"':'')+'><span class="tag">'+esc(x.tag)+'</span><h3><svg class="ic" aria-hidden="true"><use href="#'+(x.icon||'ico-doc')+'"></use></svg> '+esc(x.title)+'</h3><p class="pain">'+esc(x.problem)+'</p><div class="foot"><span class="badge '+bc+'">'+esc(x.badge)+'</span><a class="btn btn-primary" href="'+esc(x.btnHref||'index.html#zayavka')+'">'+esc(x.btnText||'Обсудить')+'</a></div></div>';}).join('');}
